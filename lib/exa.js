...
/**
 * Core Exa POST wrapper — routes through /api/exa server proxy.
 *
 * WHY: Direct browser → api.exa.ai calls are blocked by CORS on any
 * real domain (Vercel, custom domains). The proxy calls Exa
 * server-to-server, which has no CORS restrictions.
 *
...
export async function exaPost(apiKey, path, body) {
  ...
  // Strip empty arrays / nulls — Exa rejects them
  ...
  // ✅ Route through our server proxy — fixes CORS on Vercel / production
  const res = await fetch("/api/exa", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ path, body: cleanBody, apiKey }),
  });

  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || `Exa proxy error (HTTP ${res.status})`);
  }
...
export function exaFounderBackground(apiKey, founderName, startupName) {
  ...
  return cached(key, async () => {
    ...
    // ── Pull the first linkedin.com/in/ URL as the canonical profile link ──
    const linkedinResult = (data.results || []).find(r =>
      r.url && r.url.includes("linkedin.com/in/")
    );
    if (linkedinResult) {
      // Attach to the array so callers can read results.linkedinUrl
      results.linkedinUrl = linkedinResult.url.split("?")[0]; // strip tracking params
    }
...
export function exaDailyStartupNews(apiKey) {
  ...
  return cached(key, async () => {
    ...
    const data = await exaPost(apiKey, "/search", {
      ...
      type: "neural",
      numResults: 25,              // ✅ All 25 now show — AI cap removed
      includeDomains: [
        "techcrunch.com",
        "reuters.com",
        "bloomberg.com",
