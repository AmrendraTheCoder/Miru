 ...
 */
async function exaPost(apiKey, path, body) {
...
export function exaDailyStartupNews(apiKey) {
  const dateKey = new Date().toISOString().split("T")[0]; // e.g. "2025-04-29"
  const key = `news::${dateKey}`;
  return cached(key, async () => {
    const windowStart = new Date(Date.now() - 72 * 60 * 60 * 1000).toISOString();

    const data = await exaPost(apiKey, "/search", {
      query: "startup raises funding seed Series A launch new product announces 2025",
      type: "neural",
      numResults: 20,
      // Only high-quality journalism — no spam/SEO blogs
      includeDomains: [
        "techcrunch.com",
        "reuters.com",
        "bloomberg.com",
      ...
      startPublishedDate: windowStart, // ✅ Was: undefined (bug). Now: last 72h
      contents: {
        text: { maxCharacters: 1200 },
        summary: {
          query: "What startup is this about? How much money did they raise? What do they do in one sentence?",
        },
      },
      // ✅ No livecrawl on news — Exa's index is fresh enough for journalism
    });
