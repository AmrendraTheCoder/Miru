...
export function exaDailyStartupNews(apiKey) {
  // Cache key per-week so results refresh weekly
  const now = new Date();
  const weekKey = `${now.getFullYear()}-W${Math.ceil(now.getDate() / 7)}`;
  const key = `news::${weekKey}`;

  return cached(key, async () => {
    // 30-day window — gets all of the current month
    ...
    const data = await exaPost(apiKey, "/search", {
      ...
      type: "neural",
      numResults: 25,
      // High-signal journalism + newsletters only
      includeDomains: [
        "techcrunch.com",
        "reuters.com",
        "bloomberg.com",
      ...
      startPublishedDate: monthStart, // last 30 days
      contents: {
        text: { maxCharacters: 1000 },
        summary: {
          query: "What startup raised funding? How much? What do they do?",
        },
      },
    });
